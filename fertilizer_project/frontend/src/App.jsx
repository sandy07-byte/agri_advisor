...
function Home() {
  ...
  useEffect(() => {
    if (token) {
      ...
      console.log(`📊 [Home] User logged in as ${isAdmin ? 'admin' : 'farmer'}, redirecting to ${redirectPath}...`)
      nav(redirectPath)
    }
  }, [token, isAdmin, nav])
...
function Login() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const userRole = d.role || 'farmer'

      // Verify role matches selection (if admin was selected but user isn't admin)
      if (selectedRole === 'admin' && userRole !== 'admin') {
        throw new Error('You do not have admin privileges. Please login as Farmer.')
      }

      setToken(d.access_token)
      setRole(userRole)
      localStorage.setItem('role', userRole)
      setSuccess(`✅ Login successful as ${userRole === 'admin' ? 'Admin' : 'Farmer'}!`)
      // Redirect based on role - admin goes to /admin, farmer to /dashboard
      ...
      <div className="auth-card animate-in">
        <div className="auth-panel">
          <div className="auth-panel-inner">
            {success && <div className="toast-success animate-fadeIn" role="status">{success}</div>}
            <h1>Welcome back</h1>
            <p className="muted">Select your role and enter credentials to continue.</p>

            {/* Role Selector */}
            <div className="role-selector animate-slideIn">
              <button
                type="button"
            ...
            {selectedRole === 'admin' && (
              <div className="admin-hint animate-fadeIn">
                <span>🔒</span> Admin credentials required for dashboard access
              </div>
            )}

            <form onSubmit={onSubmit} className="form">
              <label>Email</label>
              <input name="email" type="email" value={form.email} onChange={onChange} required aria-invalid={!!fieldErrors.email} placeholder={selectedRole === 'admin' ? 'admin@agriadvisor.com' : 'your@email.com'} />
              ...
              <button type="submit" className="btn btn-animate" disabled={loading}>
                {loading ? 'Signing in...' : `Login as ${selectedRole === 'admin' ? 'Admin' : 'Farmer'}`}
              </button>
              {error && <div className="error animate-shake" role="alert">{error}</div>}
...
// Indian states list for registration dropdown
const INDIAN_STATES = [
...
function Register() {
  const { setToken, setRole } = useAuth()
  const nav = useNavigate()
  const [form, setForm] = useState({ name: '', email: '', password: '', phone: '', state: '', district: '', role: 'farmer' })
  const [error, setError] = useState('')
  const [fieldErrors, setFieldErrors] = useState({})
  const [loading, setLoading] = useState(false)
  const [success, setSuccess] = useState('')
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setToken(d.access_token)
      // Set role from server response (defaults to 'farmer' for registration)
      const userRole = d.role || 'farmer'
      setRole(userRole)
      localStorage.setItem('role', userRole)
      setSuccess(`✅ Registration successful! Redirecting to ${userRole === 'admin' ? 'Admin Dashboard' : 'Farmer Dashboard'}...`)
      // Redirect based on role
      const redirectPath = userRole === 'admin' ? '/admin' : '/dashboard'
      setTimeout(() => nav(redirectPath), 2000)
      ...
      <div className="auth-card auth-card--register animate-in">
        <div className="auth-panel">
          <div className="auth-panel-inner">
            ...
            {form.role === 'admin' && (
              <div className="admin-hint animate-fadeIn">
                <span>⚠️</span> Admin accounts have full dashboard access
              </div>
            )}
            ...
            <form onSubmit={onSubmit} className="form register-form">
              ...
              <div className="form-section">
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input name="name" value={form.name} onChange={onChange} placeholder="Enter your full name" required aria-invalid={!!fieldErrors.name} />
              ...
              <div className="form-section">
                ...
                <div className="form-group">
                  ...
                  <input name="email" type="email" value={form.email} onChange={onChange} placeholder="yourname@example.com" required aria-invalid={!!fieldErrors.email} />
                  {fieldErrors.email && <div className="field-error" role="alert">{fieldErrors.email}</div>}
                </div>
                <div className="form-group">
                  <label>Password *</label>
                  <input name="password" type="password" value={form.password} onChange={onChange} placeholder="Create a strong password" required aria-invalid={!!fieldErrors.password} />
                  <small className="hint">Use 8+ characters with at least one letter and one number.</small>
                  {fieldErrors.password && <div className="field-error" role="alert">{fieldErrors.password}</div>}
                </div>
              </div>
              ...
              <div className="form-section">
                ...
                <div className="form-row">
                  <div className="form-group">
                    <label>State *</label>
                    <select name="state" value={form.state} onChange={onChange} required aria-invalid={!!fieldErrors.state}>
                      <option value="">-- Select State --</option>
                      {INDIAN_STATES.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                    {fieldErrors.state && <div className="field-error" role="alert">{fieldErrors.state}</div>}
                  </div>
                  <div className="form-group">
                    <label>District *</label>
                    <input name="district" value={form.district} onChange={onChange} required placeholder="e.g., Hyderabad" aria-invalid={!!fieldErrors.district} />
                    {fieldErrors.district && <div className="field-error" role="alert">{fieldErrors.district}</div>}
                  </div>
                </div>
              </div>

              <button type="submit" className="btn btn-animate register-btn" disabled={loading}>
                {loading ? (
                  <><span className="spinner"></span> Creating account...</>
                ) : (
                  <><span className="btn-icon">{form.role === 'admin' ? '🔐' : '🌾'}</span> Register as {form.role === 'admin' ? 'Admin' : 'Farmer'}</>
                )}
              </button>
              {error && <div className="error animate-shake" role="alert">{error}</div>}
...
function ArticlesIndex() {
  ...
  const categories = ['all', ...new Set(articles.map(a => a.category || 'General').filter(Boolean))]

  const filteredArticles = articles.filter(a => {
    const matchesSearch = a.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (a.description || a.content || '').toLowerCase().includes(searchTerm.toLowerCase())
    const matchesCategory = selectedCategory === 'all' || (a.category || 'General') === selectedCategory
    return matchesSearch && matchesCategory
  })

  const isEmpty = !loading && filteredArticles.length === 0

  return (
    <div className="container articles-page fade-in">
      <div className="page-header animated-header">
        <h1>📚 Agricultural Articles</h1>
        <p className="page-subtitle">Explore the latest farming knowledge and best practices</p>
      </div>
      ...
      {isEmpty && <p className="muted empty-state">No articles match your search.</p>}
      <div className="grid-articles">
        {filteredArticles.map((a, index) => {
          ...
          return (
            <div
              key={a.id}
              className="card article-card animated-card"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="card-image-wrapper">
                <img src={a.image} alt={a.title} loading="lazy" />
                {a.category && <span className="article-category-tag">{a.category}</span>}
              </div>
              <div className="card-body">
                <h4>{a.title}</h4>
                <p>{excerpt}</p>
                <Link to={`/articles/${a.id}`} className="btn secondary read-more-btn">
